...
func addAuthorization(t *testing.T, request *http.Request, tokenMaker token.Maker, authType string, username string, role string,  duration time.Duration) {
...
func TestAuthMiddle(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			name: "OK",
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationBearer, "user", util.DepositorRole, time.Minute)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
